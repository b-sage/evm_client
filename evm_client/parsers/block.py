...
def parse_raw_block(block_dict):
    return ParsedBlock(
        ...
        extra_data=block_dict['extraData'] if block_dict.get('extraData') is not None else None,
        ...
        hash_=block_dict['hash'] if block_dict.get('hash') is not None else None,
        logs_bloom=block_dict['logsBloom'] if block_dict.get('logsBloom') is not None else None,
        miner=block_dict['miner'] if block_dict.get('miner') is not None else None,
        mix_hash=block_dict['mixHash'] if block_dict.get('mixHash') is not None else None,
        ...
        parent_beacon_block_root=block_dict['parentBeaconBlockRoot'] if block_dict.get('parentBeaconBlockRoot') is not None else None,
        parent_hash=block_dict['parentHash'] if block_dict.get('parentHash') is not None else None,
        receipts_root=block_dict['receiptsRoot'] if block_dict.get('receiptsRoot') is not None else None,
        sha3_uncles=block_dict['sha3Uncles'] if block_dict.get('sha3Uncles') is not None else None,
        size=hex_to_int(block_dict['size']) if not block_dict.get('size') is not None else None,
        state_root=block_dict['stateRoot'] if block_dict.get('stateRoot') is not None else None,
